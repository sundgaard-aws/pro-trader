...
function LoginView() {
    var _this = this;
    var LOGIN_URL = "https://9qw1dn9auc.execute-api.eu-north-1.amazonaws.com/login-fn"; 
    var CREATE_LOGIN_URL = "https://sd4dvftlwa.execute-api.eu-north-1.amazonaws.com/create-login-fn";
    var maxHeroes = 3;
    var welcomeMusic = {};

    this.buttonPushed = function(e) {
        var action = $(e.currentTarget).attr("data-action");
        if(action == "createPortfolio") { heroView.drawCreatePortfolioScreen(e.currentTarget); }
    };

    this.createLogin = function() {
        var newClientLogin = {email:$("#newLogin").val(), password:$("#newPassword").val(), passwordRepeated:$("#newRepeatedPassword").val()};
    ...
    this.login = function() {
        ...
        var clientLogin = {email:$("#login").val(), password:$("#password").val()};
        gameSession.setEmail($("#login").val());
